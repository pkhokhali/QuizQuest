...
import { getSchedulerStatus, autoGenerateDigestsForDate } from "../services/scheduler.js";
import { pushDigestNotification, broadcastPushNotification } from "../services/notifications.js";
...
router.put("/digests/:id", (req, res) => {
  ...
  const b = req.body || {};
  const newStatus = b.status === "published" || b.status === "draft" ? b.status : d.status;
  db.prepare(
    `UPDATE digests SET date = ?, grade_band = ?, headline_en = ?, headline_ne = ?, gk_fact_en = ?, gk_fact_ne = ?, nepal_fact_en = ?, nepal_fact_ne = ?, status = ? WHERE id = ?`
  ).run(
    b.date || d.date,
...
router.get("/digests/status", (req, res) => {
  const status = getSchedulerStatus();
  res.json(status);
});

router.post("/digests/:id/push", async (req, res) => {
  const d = db.prepare("SELECT * FROM digests WHERE id = ?").get(req.params.id);
  if (!d) return res.status(404).json({ error: "Digest not found" });
  ...
  try {
    const result = await pushDigestNotification({
      digest: d,
      isManual: true,
      adminEmail: req.admin?.email || "admin@quizquest",
    });
    const updated = db.prepare("SELECT * FROM digests WHERE id = ?").get(d.id);
    res.json({ ok: true, result, digest: serializeDigest(updated) });
  } catch (err) {
    console.error("Error manual pushing digest:", err);
    res.status(500).json({ error: err.message || "Failed to dispatch push notification" });
  }
});

router.post("/digests/push-today", async (req, res) => {
  const todayStr = req.body?.date || today();
  let digests = db.prepare("SELECT * FROM digests WHERE date = ? AND status = 'published'").all(todayStr);

  if (!digests.length) {
    const drafts = db.prepare("SELECT * FROM digests WHERE date = ? AND status = 'draft'").all(todayStr);
    if (drafts.length) {
      db.prepare("UPDATE digests SET status = 'published' WHERE date = ? AND status = 'draft'").run(todayStr);
      digests = db.prepare("SELECT * FROM digests WHERE date = ? AND status = 'published'").all(todayStr);
    } else {
      digests = autoGenerateDigestsForDate(todayStr);
    }
  }
  ...
  for (const d of digests) {
    try {
      const pushRes = await pushDigestNotification({
        digest: d,
        isManual: true,
        adminEmail: req.admin?.email || "admin@quizquest",
      });
      totalRecipients += pushRes.success;
    } catch (err) {
      console.error(`Error pushing digest #${d.id}:`, err);
    }
  }

  const updatedRows = db.prepare("SELECT * FROM digests WHERE date = ?").all(todayStr);
  res.json({
    ok: true,
    message: `Pushed ${digests.length} digest(s) to ${totalRecipients} device(s).`,
    digests: updatedRows.map(serializeDigest),
    recipientsCount: totalRecipients,
  });
});

router.post("/digests/auto-generate", (req, res) => {
  const dateStr = req.body?.date || today();
  const force = Boolean(req.body?.force);
  const created = autoGenerateDigestsForDate(dateStr, force);
  const rows = db.prepare("SELECT * FROM digests WHERE date = ?").all(dateStr);
  res.json({
    ok: true,
    createdCount: created.length,
    digests: rows.map(serializeDigest),
  });
});
